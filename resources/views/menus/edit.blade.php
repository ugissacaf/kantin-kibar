        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('menus.update', $menu) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" value="{{ old('name',$menu->name) }}" required />
                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                </div>
                <div class="mb-4">
                    <x-input-label for="description" :value="__('Deskripsi')" />
                    <textarea id="description" name="description" class="mt-1 block w-full border-gray-300 rounded-md">{{ old('description',$menu->description) }}</textarea>
                ...
                <div class="mb-4">
                    ...
                    <x-text-input id="daily_quota" name="daily_quota" type="number" class="mt-1 block w-full" value="{{ old('daily_quota',$menu->daily_quota) }}" required />
                    <x-input-error :messages="$errors->get('daily_quota')" class="mt-2" />
                </div>
                <div class="mb-4">
                    <x-input-label for="is_available" :value="__('Tersedia')" />
                    <input id="is_available" name="is_available" type="checkbox" class="mt-1" value="1" @checked(old('is_available',$menu->is_available)) />
                    <x-input-error :messages="$errors->get('is_available')" class="mt-2" />
                </div>
                <div class="mb-4">
                    <x-input-label for="image" :value="__('Gambar (opsional)')" />
                    @if($menu->image)
                        <div class="mb-1"><img src="{{ asset('storage/'.$menu->image) }}" class="w-32 h-32 object-cover" /></div>
                    @endif
